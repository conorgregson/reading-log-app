// Service Worker for Readr v1.2.0
const CACHE = "readr-v1.2.0";
...
self.addEventListener("install", (event) => {
    event.waitUntil(caches.open(CACHE_STATIC).then((cache) => cache.addAll(ASSETS)));
    self.skipWaiting();
});
...
self.addEventListener("fetch", (event) => {
    ...
    if (request.mode === "navigate") {
        event.respondWith((async () => {
            try {
                ...
                // Keep an up-to-date index for offline nav
    ...
    // 2. JSON → network-first (fresh backups/imports), fallback to chached/offline
